...
import { whyChooseUsService } from '../services/supabase';

const ICONS = ['Award','MapPin','TrendingUp','Building2','Users','Shield','Star','Zap','CheckCircle','Heart','Globe','Lock'];
const EMPTY = { title: '', description: '', icon: 'Award', sort_order: 0 };
...
export default function AdminWhyUs() {
  ...
  const openEdit = (item) => { setEdit(item); setForm({ title: item.title, description: item.description || '', icon: item.icon || 'Award', sort_order: item.sort_order || 0 }); setError(''); setModal(true); };
  ...
  const handleSave = async () => {
    ...
    const result = editItem
      ? await whyChooseUsService.update(editItem.id, payload)
      : await whyChooseUsService.insert(payload);
    if (result.error) { setError(result.error.message); }
    else { showToast(editItem ? 'Updated successfully.' : 'Reason added.'); close(); fetchItems(); }
    setSaving(false);
  };

  const handleDelete = async () => {
    await whyChooseUsService.delete(deleteId);
    setDelId(null);
      ...
      <div className="admin-header">
        <div>
          ...
          <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '0.82rem', marginTop: '4px' }}>
            {items.length} reason{items.length !== 1 ? 's' : ''} listed — shown on Home & About pages
          </p>
        </div>
      ...
      <div className="admin-card" style={{ padding: 0 }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '80px 40px' }}>
            <CheckSquare size={40} style={{ color: 'rgba(255,255,255,0.08)', margin: '0 auto 16px' }} />
            <p style={{ color: 'rgba(255,255,255,0.25)', marginBottom: '20px' }}>No reasons added yet.</p>
            <button className="btn" onClick={openAdd} style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', fontSize: '0.78rem' }}>
              <Plus size={14} /> Add First Reason
            </button>
          </div>
        ) : (
          ...
          <div style={{ background: '#111', border: '1px solid rgba(255,255,255,0.1)', padding: '36px', width: '100%', maxWidth: '560px', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '28px' }}>
              <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: '1.3rem' }}>
                {editItem ? 'Edit Reason' : 'Add Reason'}
              </h3>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
              <div className="form-group">
                ...
                <input className="form-input" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="e.g. Deep Expertise in Indian Markets" />
              </div>

              <div className="form-group">
                <label className="form-label">Description</label>
                <textarea className="form-input" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Brief supporting description..." style={{ minHeight: '100px' }} />
              </div>
              ...
              <div className="form-group">
                ...
                <input className="form-input" value={form.icon} onChange={(e) => setForm({ ...form, icon: e.target.value })} placeholder="Or type a Lucide icon name" />
              </div>

              <div className="form-group">
                <label className="form-label">Sort Order</label>
                <input className="form-input" type="number" min="0" value={form.sort_order} onChange={(e) => setForm({ ...form, sort_order: e.target.value })} style={{ maxWidth: 100 }} />
              </div>
      ...
      {deleteId && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: '20px' }}>
          <div style={{ background: '#111', border: '1px solid rgba(239,68,68,0.2)', padding: '36px', width: '100%', maxWidth: '400px' }}>
            <h3 style={{ marginBottom: '10px' }}>Delete this reason?</h3>
